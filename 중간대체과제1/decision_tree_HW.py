...
def entropy(target_col):
    ...
    이 함수의 유일한 매개변수는 대상 열을 지정하는 target_col 매개변수다.
    """
    elements, counts = np.unique(target_col, return_counts = True)
    #target_col안의 unique한 column만 찾아서 element에 반환하고, unique한 column의 갯수를 counts에 반환
    
    # Please implement the routine to compute the entropy given the data
    entropy = -np.sum([(counts[i]/np.sum(counts))*np.log2(counts[i]/np.sum(counts)) for i in range(len(elements))])
    #counts[i]/np.sum(counts)) 주어진 데이터 집합 에서 중 특정 클래스일 확률 구하는 식
    #np.log2(counts[i]/np.sum(counts)) 그 확률에 로그를 취함
...
def InfoGain(data, split_attribute_name, target_name="class"):
    ...
    #data[split_attribute_name]안의 unique한 data만 찾아서 vals에 반환하고, unique한 data의 갯수를 counts에 반환
    ...
    #(counts[i]/np.sum(counts))은 앞에 가중치를 구하는 공식
    #entropy(data.where(data[split_attribute_name]==vals[i]).dropna()[target_name]) 
    #각각의 피쳐에 해당하는 엔트로피를 구해서 곱해야함
    #기본문제 139줄에서 힌트를 획득
    #data.where(data[split_attribute_name]==vals[i]) data를 넣는데 어떤데이터??? 속성이름이 피쳐와 같은 데이터
    #.dropna() 결측값 제거하는 메소드
    #[target_name] data.where로 나온 리스트의 피쳐값에 접근하겠다는 의미
